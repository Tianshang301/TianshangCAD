...
- The streamable-HTTP endpoint is guarded by `TIANSHANGCAD_API_KEY` (single key) or
...
- `TIANSHANGCAD_AUTO_APPROVE` configures the auto-approved tool set;
  `TIANSHANGCAD_SAFE_MODE=true` disables destructive operations entirely.
...
- Batch jobs run Python / SCR / batch scripts in a sandboxed subprocess with
  a configurable timeout (`TIANSHANGCAD_SAFE_MODE` and batch scheduling options).
- Scripts are never executed inline in the server process.

## Configuration Checklist

- Set `TIANSHANGCAD_API_KEY` (or `TIANSHANGCAD_API_KEYS`) in production.
- Keep `TIANSHANGCAD_AUTO_APPROVE` as small as possible.
- Enable `TIANSHANGCAD_SAFE_MODE=true` when destructive operations are not needed.
