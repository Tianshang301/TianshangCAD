...
mirroring :class:`~cad_mcp_server.core.kernel.AnalyticKernel` so the
...
class OCCTKernel(CADKernel):
    ...
    def create_box(
        ...
        if rotation is not None:
            ...
            if len(rotation_list) == 3 and all(
                ...
                from cad_mcp_server.core.transform import (
                    compose,
                    rotation_x,
                    rotation_y,
                    rotation_z,
    ...
    def transform(self, shape: Shape, matrix: Any) -> Shape:
        ...
        from cad_mcp_server.core.transform import apply_point, uniform_scale
