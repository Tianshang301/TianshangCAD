...
``cad_mcp_server.core.solver`` as the primary path when planegcs is
installed.
...
def _requires_planegcs() -> None:
    if Sketch is None:
        raise PlanegcsUnavailableError(
            ...
            "cad-mcp-server[solver]` (requires Python >=3.12)",
...
def solve_2d_planegcs(
    ...
    *,
    ...
    Mirrors the shape of :func:`cad_mcp_server.core.solver.solve_2d`:
