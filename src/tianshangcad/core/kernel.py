...
``TIANSHANGTIANGSHANGCAD_RUNTIME`` setting.
"""
...
class AnalyticKernel(CADKernel):
    ...
    def _rotation_to_matrix(rotation: Sequence[float]) -> list[list[float]]:
        from tianshangcad.core.transform import compose, rotation_x, rotation_y, rotation_z
    ...
    def _transform_params(self, kind: str, params: dict[str, Any], matrix: Any) -> dict[str, Any]:
        from tianshangcad.core.transform import apply_point, apply_points, uniform_scale
...
def get_kernel(runtime: str | None = None) -> CADKernel:
    """Return the kernel configured by ``TIANSHANGTIANGSHANGCAD_RUNTIME`` (default analytic)."""
    ...
    raise CADValidationError(
        f"Unknown TIANSHANGTIANGSHANGCAD_RUNTIME {selected!r}. Supported: analytic, ocp, freecad",
